...
lib/
  ...
  pgtx/                один запрос = одна транзакция: api.authorize → api.* → api.log_request → COMMIT | ROLLBACK
...
`platform.New(cfg, modules…)` собирает модули в один `http.Handler`, который обслуживает процесс. До любого маршрута он проверяет bearer-токен ключами `cfg.Keys`, устанавливает сессию (`code` = `sub` из JWT, агент и хост — из заголовков, которые пересылает шлюз), возвращает `X-Request-Id` без изменений, считает запрос в полёте и разбирает строку запроса; два модуля с одним префиксом отвергаются на старте, а не в `/register`. `platform.Prefixes(modules…)` — объединение префиксов в порядке регистрации, без префиксов, вложенных в другой префикс того же процесса: шлюз маршрутизирует по самому длинному префиксу и отвергает пересечение.
...
BEGIN
  ...
  SELECT api.log_request(…)                              -- если база его имеет
COMMIT
```
